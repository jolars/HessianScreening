...
write_rds(g, "results/response-pathlength.rds")

source("R/utils.R")

d_raw <- read_rds("results/response-pathlength.rds") %>%
  mutate(
  ...
  select(np, n, p, rho, path_length, screening_type, time) %>%
  unnest(time)
...
d1 <-
  ...
  group_by(np, path_length, screening_type) %>%
...
d1 %>%
  ...
  rename("path length" = "path_length", "scenario" = "np") %>%
