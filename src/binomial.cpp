...
{
  return -sum(y % Xbeta - arma::log1p(expXbeta)) +
...
Binomial::dual()
{
  return -arma::sum(pr % arma::log(pr) + (1 - pr) % arma::log(1 - pr));
}

double
Binomial::scaledDual(const double lambda, const double dual_scale)
{
...
double
Binomial::dual(const double lambda, const arma::vec& theta)
{
  using namespace arma;
