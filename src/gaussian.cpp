...
double
Gaussian::dual(const double lambda, const arma::vec& theta)
{
...
double
Gaussian::scaledDual(const double lambda, const double dual_scale)
{
  if (dual_scale == 0) {
    return 0;
  } else {
    ...
    return alpha * dot(residual, y) - 0.5 * std::pow(alpha * norm(residual), 2);
  }
}
...
{
  arma::uvec inactive_restricted = setDiff(restricted_set, active_set);

  const arma::vec tmp = X.cols(active_set) * Hinv_s;
