...
The results from our paper were run through a singularity container. The
singularity container will be published alongside the paper upon
acceptance but couldn’t be included in the submission due to size
constraints.
...
The datasets used in this project could also not be submitted due to
space constraints. They can, however, be downloaded by running the
scripts in the [data-raw folder](data-raw/).

### Re-building the Singularity Container

If you want to re-build the singularity container from scratch, you can
do so by navigating to the root of the repo and build the singularity
...
Please do, however, first make sure that you’ve downloaded all of the
datasets.
...
activate the renv repository) and then running
